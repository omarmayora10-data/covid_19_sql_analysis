...
SELECT TOP 1 City, (COUNT(CASE WHEN Has_Covid = 'Yes' THEN 1 END) * 1.0 / COUNT(*)) AS positive_rate
FROM covid_19
GROUP BY City
ORDER BY City DESC;
...
SELECT CASE WHEN Fever < 100 THEN 'Normal'
        WHEN Fever BETWEEN 100 AND 101.9 THEN 'Mild'
        ELSE 'High' END AS fever_range,
    COUNT(*) AS total_patients,
    COUNT(CASE WHEN Has_Covid = 'Yes' THEN 1 END) AS positive_cases
FROM covid_19
GROUP BY CASE WHEN Fever < 100 THEN 'Normal'
        WHEN Fever BETWEEN 100 AND 101.9 THEN 'Mild'
        ELSE 'High' END;

--Calculate running totals of COVID-positive cases ordered by age
SELECT Age, COUNT(*) OVER (ORDER BY Age ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_total
FROM covid_19
WHERE Has_Covid = 'Yes';

--Identify duplicate patient records
SELECT Age, Gender, City, Fever, Cough, COUNT(*) AS duplicate_count
FROM covid_19
GROUP BY Age, Gender, City, Fever, Cough
HAVING COUNT(*) > 1;

--Compare average fever between COVID-positive and COVID-negative patients
SELECT Has_Covid, AVG(Fever) AS avg_fever
FROM covid_19
GROUP BY Has_Covid;

--Find cities where more than 30% of patients are COVID-positive
SELECT City, COUNT(CASE WHEN Has_Covid = 'Yes' THEN 1 END) * 1.0 / COUNT(*) AS positivity_rate
FROM covid_19
GROUP BY City
...
SELECT CASE WHEN Fever < 100 THEN 'Normal'
        WHEN Fever BETWEEN 100 AND 101.9 THEN 'Mild'
        ELSE 'High' END AS fever_category,
    COUNT(*) AS total_patients,
    COUNT(CASE WHEN Has_Covid = 'Yes' THEN 1 END) AS positive_cases
FROM covid_19
GROUP BY CASE WHEN Fever < 100 THEN 'Normal'
